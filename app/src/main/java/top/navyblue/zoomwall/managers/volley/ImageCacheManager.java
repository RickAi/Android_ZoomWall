package top.navyblue.zoomwall.managers.volley;
...
import top.navyblue.zoomwall.models.common.ZoomWallApplication;
...
public class ImageCacheManager {
    ...
    // 取运行内存阈值的1/8作为图片缓存
    private static final int MEM_CACHE_SIZE = 1024 * 1024 * ((ActivityManager) ZoomWallApplication.getContext().getSystemService(Context.ACTIVITY_SERVICE)).getMemoryClass() / 8;
                    ...
                    if (!isImmediate && defaultImageDrawable != null) {
                        TransitionDrawable transitionDrawable = new TransitionDrawable(
                                new Drawable[]{
                                        ...
                                        new BitmapDrawable(ZoomWallApplication.getContext().getResources(),
                                                response.getBitmap())
                                }
                        );
